/**
 * The chat composer: textarea that grows with content, file upload,
 * submit, stop, model selector. Used by ChatPanel.
 */
import {
  ChevronDown,
  FileText,
...
import type { AttachmentMeta, ChatConfig, ModelDescriptor } from "../../types/chat.js";
import { cn, formatBytes, uid } from "../../lib/utils.js";

export interface ChatComposerProps {
  className?: string;
  /** Render override for the trailing "send" button. */
  renderSendButton?: (props: { disabled: boolean; onClick: () => void }) => React.ReactNode;
}

export function ChatComposer({ className, renderSendButton }: ChatComposerProps) {
  const chat = useChat();
  ...
  const presets = useMemo(
    () =>
      Object.values(defaultModelPresets).filter(
        (m) => (cfg.model.provider ?? cfg.provider.kind) === (m.provider ?? cfg.provider.kind),
      ),
    ...
    async (files: FileList | File[]) => {
      try {
        ...
        chat.config.onError?.(
          {
            code: "bad_request",
            message: (err instanceof Error ? err.message : String(err)) ?? "File upload failed",
            retryable: false,
          },
      ...
      <div className="flex items-end gap-2">
        ...
        <div className="flex-1">
          <Textarea
            ...
            // Auto-grow: matches shadcn-style composer behaviour.
            onInput={(e) => {
        ...
        {isStreaming ? (
          ...
          renderSendButton({ disabled: !canSend, onClick: () => void onSubmit() })
        ) : (
          <Button
            type="submit"
            size="icon"
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          <span className="max-w-[120px] truncate font-mono text-[11px]">{currentModelId}</span>
